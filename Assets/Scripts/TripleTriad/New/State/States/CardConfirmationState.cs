...
public class CardConfirmationState : TtState
{
    ...
    {
        if ((Input.GetKeyDown(KeyCode.D)|| Input.GetButtonDown("right")) && ttMan.ttLogic.CanIScrollOnCardConfirm(WhichWayToScroll.IncrementingFinger))
        {
            CardConfirmLeftRightScroll(WhichWayToScroll.IncrementingFinger);
        }
        else if ((Input.GetKeyDown(KeyCode.A) || Input.GetButtonDown("left")) && ttMan.ttLogic.CanIScrollOnCardConfirm( WhichWayToScroll.DecrementingFinger))
        {
            CardConfirmLeftRightScroll(WhichWayToScroll.DecrementingFinger);
        }
        else if ((Input.GetKeyDown(KeyCode.C) || Input.GetButtonDown("Fire2")) && ttMan.ttLogic.CanIPressCancelButtonOnConfirmScreen())
        {
            ChangeToGoBackOnCancelButton();
        }
        else if ((Input.GetKeyDown(KeyCode.Space) || Input.GetButtonDown("Fire1")) && ttMan.ttLogic.CanIGoBackToCardSelectionScreenFromConfirmScreen())
        {
            ttMan.SendChangeToPreviousState();
        }
        else if ((Input.GetKeyDown(KeyCode.Space) || Input.GetButtonDown("Fire1"))&& ttMan.ttLogic.CanIAdvanceToChooseEnemyHandScreen())
        {
            ttMan.SendStateChange(ttMan.enemyHandSelectionState);
        }
        
        ttMan.ttUi.KeepFingerOnProperLocationInCardConfirmation(ttMan.ttDb.RetrieveCurrentFingerPositionInCardConfirmation());   
    }
