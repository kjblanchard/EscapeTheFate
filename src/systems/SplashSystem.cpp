...
constexpr float kFadeInTime = 0.75f;
constexpr float kHoldTime = 1.0f;
...
void advanceToNextLogo() {
	...
	if (currentLogoIndex_ >= (int)logos_.size()) {
		...
		if (!titlePanel || !menuPanel) sgLogCritical("Can not do it!");
...
void SplashSystem::Start() {
	...
	for (const auto& name : config.logos) {
		...
		while (texW / (d + 1) > 0 && texH / (d + 1) > 0 &&
			   (texW / d > kLogicalW || texH / d > kLogicalH)) {
			++d;
		}
		float drawW = (float)(texW / d);
		float drawH = (float)(texH / d);
		float drawX = (kLogicalW - drawW) / 2.0f;
		float drawY = (kLogicalH - drawH) / 2.0f;
...
void SplashSystem::Update() {
	...
	switch (state_) {
		case SplashState::FadingIn: {
			float alpha = Engine::Tweening::GetTweenedValue(0.0f, 255.0f, elapsed_, kFadeInTime);
			currentAlpha_ = (uint8_t)(alpha > 255.0f ? 255 : alpha);
			if (elapsed_ >= kFadeInTime) {
				currentAlpha_ = 255;
				elapsed_ = 0.0f;
		...
		case SplashState::Holding:
			if (elapsed_ >= kHoldTime) {
				elapsed_ = 0.0f;
				state_ = SplashState::FadingOut;
			}
			break;
		case SplashState::FadingOut: {
			float alpha = Engine::Tweening::GetTweenedValue(255.0f, 0.0f, elapsed_, kFadeOutTime);
			currentAlpha_ = (uint8_t)(alpha < 0.0f ? 0 : alpha);
			if (elapsed_ >= kFadeOutTime) {
				currentAlpha_ = 0;
				advanceToNextLogo();
			}
			break;
