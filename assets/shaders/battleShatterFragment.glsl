...
void main() {
    ...
    // Determine which column this pixel is in
    float col = floor(uv.x * COLS);

    // Search all rows in this column to find which shard has fallen to cover this pixel
    ...
    for (int r = 0; r < 5; r++) {
        ...
        if (uv.y >= cellTop && uv.y < cellBot && uv.x >= cellLeft && uv.x < cellRight) {
            // This cell has fallen to cover our pixel — sample from original position
    ...
    // Voronoi crack overlay (visible before shards separate)
    ...
    // Grid edge lines (thin dark lines between cells, visible while shards are on screen)
    if (found) {
