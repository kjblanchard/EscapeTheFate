  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = [
        ...
        "${aws_s3_bucket.supergoon_world_bucket.arn}/*",
        aws_s3_bucket.supergoon_world_bucket_dev.arn,
        "${aws_s3_bucket.supergoon_world_bucket_dev.arn}/*",
      ],
    }],
  })
...
resource "aws_s3_bucket" "supergoon_world_bucket_dev" {
  bucket = "etf-dev-external-site"
  tags = {
    Name = "Etf External Site Dev Bucket"
  }
}

resource "aws_s3_bucket_acl" "bucket_dev_acl" {
  bucket     = aws_s3_bucket.supergoon_world_bucket_dev.id
  depends_on = [aws_s3_bucket_ownership_controls.s3_bucket_acl_ownership]
  acl        = "private"
}

resource "aws_s3_bucket_ownership_controls" "s3_bucket_acl_ownership_dev" {
  bucket = aws_s3_bucket.supergoon_world_bucket_dev.id
  rule {
    object_ownership = "ObjectWriter"
  }
