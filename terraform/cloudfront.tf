...
resource "aws_cloudfront_distribution" "escape_the_fate_dev_distribution" {
  ...
  enabled             = true
  default_root_object = "EscapeTheFate.html" # The main HTML file for your website
  aliases             = ["escapethefate-dev.supergoon.com"]
...
resource "aws_s3_bucket_policy" "cloudfront_access_policy_dev" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      Action   = "s3:GetObject",
      Effect   = "Allow",
      Resource = "${aws_s3_bucket.supergoon_world_bucket_dev.arn}/*",
      Principal = {
        Service = "cloudfront.amazonaws.com"
      }
      Condition = {
        StringEquals = {
          "AWS:SourceArn" = aws_cloudfront_distribution.escape_the_fate_dev_distribution.arn
        }
      }
    }]
  })
